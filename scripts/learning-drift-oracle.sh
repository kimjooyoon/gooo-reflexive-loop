...
jq -S -n --slurpfile before "$tmp/before.json" --slurpfile after "$tmp/after.json" \
  '{schema:"gooo/reflexive-loop/learning-drift-gated/oracle-verdict/v1",method:"independent_required_activity_oracle",decision:(if $after[0].oracle_failures > 0 then "REFUTED" else "CLOSED" end),equivalent:($before[0].oracle_failures > 0 and $after[0].oracle_failures == 0),oracle_failures:{before:$before[0].oracle_failures,after:$after[0].oracle_failures},missing_required_activities:{before:$before[0].missing_required_activities,after:$after[0].missing_required_activities},counterexamples:(if $after[0].oracle_failures > 0 then ["REQUIRED_ACTIVITY_STILL_MISSING"] else [] end)}'
