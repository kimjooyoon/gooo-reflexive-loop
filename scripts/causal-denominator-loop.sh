...
if ! jq -e --arg tag "$causal_tag" --arg target "$causal_target" \
  '.immutable==true and .tag_name==$tag and (.target_commitish=="main" or .target_commitish==$target)' \
...
if ! jq -e --arg tag "$denominator_tag" --arg target "$denominator_target" \
  '.immutable==true and .tag_name==$tag and .target_commitish==$target' "$denominator_release" >/dev/null 2>&1 ||
