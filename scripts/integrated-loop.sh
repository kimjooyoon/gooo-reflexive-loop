...
jq -S -n --arg scenario "$scenario" --arg report_digest "$(digest "$output/report.json")" \
  ...
  '{schema:"gooo/reflexive-loop/integrated-ci-artifact/v1",scenario:$scenario,decision:$report[0].decision,summary:$report[0].summary,report_digest:$report_digest,external_inputs:$report[0].external_inputs,resolution_unknown:$report[0].resolution_unknown,execution_plan:$report[0].proposal.execution_plan,lifecycle:$report[0].lifecycle,authority:$report[0].authority,bindings:$report[0].bindings,metrics:$report[0].cells,artifact:{files:$artifact_files,bytes:$artifact_bytes},repository_unchanged:$report[0].authority.input_repository_unchanged}' \
